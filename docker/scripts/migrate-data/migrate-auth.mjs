/**
 * Supabase Auth Migration Script — Cloud → self-hosted stack
 *
 * Copy of migration_dump/migrate-auth.mjs, retargeted at this migration
 * (Supabase Cloud -> the self-hosted GoTrue running in Docker) with its own
 * mapping file, so it doesn't touch migration_dump/auth-id-mapping.json
 * (that one documents a PAST cloud-to-cloud project migration).
 *
 * Migrates all auth.users from the source project to the destination via
 * the Admin API.
 *
 * Limitations:
 ...
 *     at docker/scripts/migrate-data/auth-id-mapping.json — fix-auth-id-references.mjs
 *     uses it to update FK references in the public schema after the data migration.
 *
 * Usage:
 *   OLD_URL=https://xxx.supabase.co OLD_KEY=service_role_key_old \
 *   NEW_URL=http://localhost:8000 NEW_KEY=service_role_key_new \
 *   node docker/scripts/migrate-data/migrate-auth.mjs
 */
...
Missing environment variables. Run with:
  ...
  NEW_KEY=<new self-hosted service role key> \\
  node docker/scripts/migrate-data/migrate-auth.mjs
`);
  process.exit(1);
}
...
async function main() {
  ...
  console.log('  Supabase Auth Migration (Cloud -> self-hosted)');
  ...
  console.log(`
Next steps:
  1. Send password reset emails to all migrated users (or have them use "Forgot Password").
  2. Run migrate-public-data.sh to copy the public schema's row data.
  3. Run fix-auth-id-references.mjs using this mapping file to update
     store_customers.auth_user_id / store_subscriptions.user_id etc.
`);
