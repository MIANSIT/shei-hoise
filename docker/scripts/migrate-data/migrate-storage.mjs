/**
 * Supabase Storage Migration Script — Cloud → self-hosted stack
 *
 * Copy of migration_dump/migrate-storage.mjs (logic unchanged — it's already
 * generic over any OLD_URL/NEW_URL pair). Copies all buckets + files from the
 ...
 *   node docker/scripts/migrate-data/migrate-storage.mjs
 */
...
async function main() {
  ...
  console.log('  Supabase Storage Migration (Cloud -> self-hosted)');
