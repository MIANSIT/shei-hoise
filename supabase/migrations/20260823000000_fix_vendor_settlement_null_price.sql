-- record_vendor_settlement did v_receivable := v_sold * v_unit_price with no
-- null/positivity check. A submitted item with no unit_price made that
-- line's contribution NULL, which then poisoned the running total for
-- every item after it in the same loop -- the settlement was saved with
-- total_receivable = NULL, silently understating what the vendor owes.
-- Now COALESCEs a missing price to 0 and fails loudly instead if a sold
-- quantity is submitted with no usable price, rather than silently
-- zeroing the receivable.
--
-- Same 8-argument signature as the 20260713000000 version -- CREATE OR
-- REPLACE only, no DROP needed.
