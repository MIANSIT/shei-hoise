    ...
    const fetchCartData = async () => {
      try {
        ...
        if (useZustand) {
          ...
          const compactProducts: Array<[string, string | undefined, number]> =
            JSON.parse(decompressed);

          if (
            !compactProducts ||
            !Array.isArray(compactProducts) ||
          ...
          const groupedCompactProducts = compactProducts.reduce(
            (acc, product) => {
          ...
          const totalItems = groupedEnrichedItems.reduce(
            (sum, item) => sum + item.quantity,
            0
          );
          const subtotal = groupedEnrichedItems.reduce(
            (sum, item) => sum + item.displayPrice * item.quantity,
            0
          );
          const totalDiscount = groupedEnrichedItems.reduce((sum, item) => {
            return (
              sum + (item.originalPrice - item.displayPrice) * item.quantity
            );
          }, 0);
