...
export default function AnimatedInvoice({ 
  ...
  // Use the invoice data hook to get complete store information
  ...
  const handlePrint = () => {
    ...
    setTimeout(() => {
      ...
      if (printWindow) {
        ...
        const customerEmail = (enhancedOrderData.shipping_address as any)?.email || enhancedOrderData.customers?.email;
        ...
        const content = `
          ...
          <html>
            ...
            <body>
              <div class="receipt">
                <div class="header">
                  <div class="business-name">${enhancedOrderData.stores?.store_name?.toUpperCase() || 'STORE'}</div>
                  <div>${enhancedOrderData.stores?.business_address || 'Business Address Not Available'}</div>
                  <div>${enhancedOrderData.stores?.contact_phone ? `Tel: ${enhancedOrderData.stores.contact_phone}` : ''}</div>
                  <div>${enhancedOrderData.stores?.contact_email ? `Email: ${enhancedOrderData.stores.contact_email}` : ''}</div>
                </div>
                
                <div class="divider">......</div>

                <div class="customer-info">
                  <div><strong>ORDER #:</strong> ${enhancedOrderData.order_number}</div>
                  <div><strong>Customer:</strong> ${enhancedOrderData.shipping_address?.customer_name || 'N/A'}</div>
                  <div><strong>Phone:</strong> ${enhancedOrderData.shipping_address?.phone || 'N/A'}</div>
                  ${customerEmail ? `<div><strong>Email:</strong> ${customerEmail}</div>` : ''}
                  <div><strong>Address:</strong> ${enhancedOrderData.shipping_address?.address_line_1 || 'N/A'}, ${enhancedOrderData.shipping_address?.city || 'N/A'}, ${enhancedOrderData.shipping_address?.country || 'N/A'}</div>
                </div>
                
                <div class="divider">......</div>
                
                <div class="items">
                  ${enhancedOrderData.order_items.map(item => {
                    const productName = item.product_name.length > 20 
                      ? item.product_name.substring(0, 20) + '...' 
                      : item.product_name;
                    return `
                      <div class="item-row">
                        <div class="item-name">${productName} x${item.quantity}</div>
                ...
                <div style="text-align: center; margin: 10px 0;">
                  <div>${enhancedOrderData.payment_method?.toUpperCase() || 'CASH'}</div>
                ...
                <div class="footer">
                  ...
                  <div>${enhancedOrderData.stores?.store_slug ? `www.shei-hoise.vercel.app/${enhancedOrderData.stores.store_slug}` : 'www.shei-hoise.vercel.app'}</div>
                </div>
              </div>
            </body>
          </html>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-primary-foreground/90 text-sm">Order #: {enhancedOrderData.order_number}</p>
              </div>
            </div>
            {showCloseButton && (
              ...
              >
                ...
                <div className="text-center mb-4 border-b border-dashed border-border pb-3">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {enhancedOrderData.stores?.business_address || 'Business Address Not Available'}
                  </div>
                ...
                <div className="mb-3 text-xs border-b border-dotted border-border pb-2">
                  <div className="text-foreground"><strong>ORDER #:</strong> {enhancedOrderData.order_number}</div>
                  <div className="text-foreground mt-2"><strong>Customer:</strong> {enhancedOrderData.shipping_address?.customer_name || 'N/A'}</div>
                  <div className="text-muted-foreground"><strong>Phone:</strong> {enhancedOrderData.shipping_address?.phone || 'N/A'}</div>
                  {customerEmail && (
                    <div className="text-muted-foreground"><strong>Email:</strong> {customerEmail}</div>
                  )}
                  <div className="text-muted-foreground mt-1">
                    <strong>Address:</strong> {enhancedOrderData.shipping_address?.address_line_1 || 'N/A'}, {enhancedOrderData.shipping_address?.city || 'N/A'}, {enhancedOrderData.shipping_address?.country || 'N/A'}
                  </div>
                </div>
                ...
                <div className="text-center space-y-1 mb-3">
                  <div className="font-medium text-foreground">{enhancedOrderData.payment_method?.toUpperCase() || 'CASH'}</div>
                  <div className="text-xs text-muted-foreground">Date: {formatDate(enhancedOrderData.created_at)}</div>
                  <div className="text-xs text-muted-foreground capitalize">Status: {enhancedOrderData.status}</div>
                </div>
