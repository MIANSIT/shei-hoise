...
const STATUS_STYLES = {
  ...
  neutral: { bg: "bg-muted", text: "text-muted-foreground", dot: "bg-gray-400" },
} as const;
      ...
      <table className="w-full text-xs min-w-160">
        <thead>
          <tr className="border-b border-border">
            <th className="text-left font-semibold text-muted-foreground pb-2 pr-4">
              {t.admin.pixelNameCol}
            </th>
            {eventMeta.map((m) => (
              <th
                key={m.key}
                className="text-right font-semibold text-muted-foreground pb-2 px-2 whitespace-nowrap"
              >
                {m.label}
              </th>
            ))}
            <th className="text-right font-semibold text-muted-foreground pb-2 px-2 whitespace-nowrap">
              {t.admin.pixelRevenueCol}
            </th>
            <th className="text-right font-semibold text-muted-foreground pb-2 pl-2 whitespace-nowrap">
        ...
        <tbody className="divide-y divide-gray-50 dark:divide-gray-800/60">
          {rows.map(([name, stats]) => (
            ...
            >
              <td className="py-2.5 pr-4 font-medium text-foreground max-w-45">
              ...
              {eventMeta.map((m) => (
                <td
                  key={m.key}
                  className="py-2.5 px-2 text-right tabular-nums text-foreground"
                >
    ...
    <div className="flex items-start gap-3 bg-indigo-50 dark:bg-indigo-500/10 rounded-xl p-3.5 mb-4">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="mt-2 flex flex-col sm:flex-row sm:items-stretch gap-2">
          <code className="flex-1 min-w-0 text-[11px] bg-card border border-indigo-200 dark:border-indigo-500/30 text-foreground rounded-lg px-3 py-2 font-mono leading-relaxed break-all">
      ...
      <div className="flex items-start gap-3 mb-4">
        ...
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            <h2 className="text-sm font-bold text-foreground">{t.admin.pixelCapiSectionTitle}</h2>
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center gap-3 bg-amber-50 dark:bg-amber-500/10 rounded-xl p-4">
      ...
      <div className="flex-1">
        <p className="text-sm font-semibold text-foreground">{t.admin.pixelCapiConnectPromptTitle}</p>
        <p className="text-xs text-muted-foreground mt-0.5">{t.admin.pixelCapiConnectPromptDesc}</p>
      </div>
      <Link
                  ...
                  return (
                    <div key={meta.key} className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 text-xs">
                      <div className="flex items-center gap-2">
                        <span className="w-24 sm:w-28 shrink-0 text-muted-foreground truncate">{meta.label}</span>
  ...
  return (
    <div className="max-w-350 mx-auto px-3 sm:px-6 py-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={load}
            disabled={loading}
            className="p-2 rounded-xl border border-border bg-card text-gray-500 hover:text-indigo-500 transition-colors"
          >
      ...
      <div className="flex items-center gap-2 pt-2">
        ...
        <h2 className="text-sm font-bold text-foreground">{t.admin.pixelSectionMetaPixelTitle}</h2>
        <span className="text-[11px] text-muted-foreground">{t.admin.pixelSectionMetaPixelSubtitle}</span>
      </div>
        ...
        {EVENT_META.map((meta) => {
          ...
          return (
            ...
            >
              ...
              <div className="w-8 h-8 rounded-xl bg-card/50 flex items-center justify-center mb-3 shadow-sm">
      ...
      <div className="rounded-2xl border border-border bg-card p-5 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-3 mt-4 pt-4 border-t border-dashed border-border">
          <div>
            ...
            <p className="text-[11px] text-muted-foreground mt-0.5">{t.admin.pixelCartToBuyRate}</p>
          </div>
          <div>
            <p className="text-lg font-black text-foreground">
              {loading ? "—" : convRate(data?.totalOrders ?? 0, totals.ViewContent ?? 0)}
            </p>
            <p className="text-[11px] text-muted-foreground mt-0.5">{t.admin.pixelViewToBuyRate}</p>
          </div>
        </div>
      </div>
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-gray-800/60">
                {(data.topProducts as ProductStat[]).map((p) => (
                  <tr
                    ...
                    className="hover:bg-background/40 transition-colors"
                  >
                    <td className="py-2.5 pr-4 font-medium text-foreground max-w-48">
                      <span className="flex items-center gap-2">
      ...
      {data?.campaigns && Object.keys(data.campaigns).length > 0 ? (
        ...
        !loading && (
          <div className="rounded-2xl border border-dashed border-border bg-background/30 p-5">
