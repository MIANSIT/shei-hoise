    ...
    <div className="max-w-[1400px] mx-auto px-3 sm:px-6 py-6 space-y-6">
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-3xl sm:text-4xl font-black tabular-nums">
              ৳ {(data?.totalRevenue ?? 0).toLocaleString("en-BD", { minimumFractionDigits: 2 })}
            </p>
            <p className="text-indigo-200 text-xs mt-1">
              From {totals.Purchase ?? 0} purchases via pixel
            </p>
          </div>
          <div className="flex items-center gap-3">
            <div className="text-center bg-white/10 rounded-xl px-4 py-3">
              <p className="text-lg font-black">
                {totals.Purchase
                  ? `${(((totals.Purchase ?? 0) / (totals.PageView || 1)) * 100).toFixed(2)}%`
                  : "—"}
              </p>
            ...
            <div className="text-center bg-white/10 rounded-xl px-4 py-3">
              <p className="text-lg font-black">
                {totals.Purchase && data?.totalRevenue
                  ? `৳ ${(data.totalRevenue / totals.Purchase).toFixed(0)}`
                  : "—"}
              </p>
              <p className="text-indigo-200 text-[10px] font-medium mt-0.5">
                Avg Order
      ...
      <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-5 shadow-sm">
        <div className="flex items-center gap-2 mb-5">
          <div className="w-1 h-5 rounded-full bg-linear-to-b from-indigo-400 to-violet-500" />
      ...
      <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-5 shadow-sm">
        <div className="flex items-center gap-2 mb-5">
          <div className="w-1 h-5 rounded-full bg-linear-to-b from-blue-400 to-indigo-500" />
          <h2 className="text-sm font-bold text-gray-900 dark:text-white">
            Daily Trend
          </h2>
        </div>
      ...
      <div className="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-5 shadow-sm">
        <div className="flex items-center gap-2 mb-4">
          <div className="w-1 h-5 rounded-full bg-linear-to-b from-emerald-400 to-teal-500" />
          <h2 className="text-sm font-bold text-gray-900 dark:text-white">
            Revenue Breakdown
          </h2>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            {
              label: "Total Revenue",
              value: `৳ ${(data?.totalRevenue ?? 0).toLocaleString("en-BD", { minimumFractionDigits: 0 })}`,
              icon: DollarSign,
              color: "text-emerald-500",
              bg: "bg-emerald-50 dark:bg-emerald-500/10",
            },
            {
              ...
              value: (totals.Purchase ?? 0).toLocaleString(),
              icon: CheckCircle,
              color: "text-blue-500",
              bg: "bg-blue-50 dark:bg-blue-500/10",
            },
            ...
            {
              label: "Cart→Buy Rate",
              value: convRate(totals.Purchase ?? 0, totals.AddToCart ?? 0),
              icon: ShoppingCart,
              color: "text-amber-500",
              bg: "bg-amber-50 dark:bg-amber-500/10",
            },
