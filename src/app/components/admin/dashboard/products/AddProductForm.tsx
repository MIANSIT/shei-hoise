...
import { adminProductSchema, ProductFormValues } from "../../../../../lib/utils/formSchema";
...
const ErrorMessage: React.FC<{ field: keyof ProductFormValues; form: UseFormReturn<ProductFormValues> }> = ({
  field,
  form,
}) => {
...
const AddProductForm: React.FC<ProductPageFormProps> = ({ product, onSubmit }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-gray-900 rounded-lg text-gray-100 shadow-lg">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        />
        <ErrorMessage field="title" form={form} />
        ...
        />
        <ErrorMessage field="category" form={form} />

        {/* Prices */}
        <PriceFields
          currentPrice={form.watch("currentPrice")}
          originalPrice={form.watch("originalPrice")}
          onChange={(e) => form.setValue(e.target.name as keyof ProductFormValues, e.target.value)}
        />
        <ErrorMessage field="currentPrice" form={form} />
        <ErrorMessage field="originalPrice" form={form} />
        ...
        <StockFields
          ...
          onChange={(e) => form.setValue(e.target.name as keyof ProductFormValues, e.target.value)}
        />
        <ErrorMessage field="discount" form={form} />
        <ErrorMessage field="stock" form={form} />
        ...
        <ErrorMessage field="images" form={form} />
