...
import {
  getVendorStoreReviews,
  VendorStoreReviewItem,
} from "@/lib/queries/storeReviews/getVendorStoreReviews";
...
function CategoryChip({ label, value }: { label: string; value: number | null }) {
...
const StoreReviewsTable: React.FC = () => {
  const { user } = useCurrentUser();
  const [notif, contextHolder] = notification.useNotification();

  const [reviews, setReviews] = useState<VendorStoreReviewItem[]>([]);
  ...
  const fetchReviews = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await getVendorStoreReviews({
  ...
  const handleToggle = async (review: VendorStoreReviewItem) => {
    setTogglingId(review.id);
    try {
      const result = await toggleStoreReviewApproval(review.id, !review.is_approved);
      if (!result.success) {
              ...
              <tr key={review.id} className="border-b border-border last:border-0 align-top">
                ...
                <td className="px-4 py-3">
                  <div className="flex flex-wrap gap-1 max-w-[180px]">
                    <CategoryChip label="Quality" value={review.product_quality_rating} />
                    <CategoryChip label="Delivery" value={review.delivery_rating} />
                    <CategoryChip label="Service" value={review.service_rating} />
                    <CategoryChip label="Value" value={review.value_rating} />
                  </div>
                </td>
...
export default StoreReviewsTable;
