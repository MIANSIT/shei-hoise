...
const PAYMENT_METHOD_OPTIONS: { value: PaymentMethod; label: string }[] = [
  ...
  { value: PaymentMethod.MOBILE_BANKING, label: "Mobile Banking (bKash/Nagad/Rocket)" },
];
...
interface CustomerQuickPaymentModalProps {
  open: boolean;
  submitting: boolean;
  customerName?: string;
  totalDue: number;
  orderOptions: CustomerOrderOption[];
  onSubmit: (payload: {
    paymentDate: string;
    amount: number;
...
function CustomerQuickPaymentModal({
  open,
  submitting,
  customerName,
  totalDue,
  ...
  onSubmit,
  onCancel,
}: CustomerQuickPaymentModalProps) {
      ...
      title={
        <div className="flex items-center gap-3 px-6 pt-5 pb-4 border-b border-gray-100 dark:border-gray-700">
          ...
          <span className="text-base font-bold text-gray-900 dark:text-white">
            Collect Payment{customerName ? ` — ${customerName}` : ""}
          </span>
        </div>
      }
      footer={
    ...
    >
      <div className="px-6 pt-5 pb-4 space-y-4">
        <p className="text-xs text-gray-400 dark:text-gray-500 -mt-1">
          Total due: {totalDue.toFixed(2)}. Log what this customer paid off just now.
        </p>
        <div>
        ...
        {orderOptions.length > 0 && (
          <div>
            ...
            <Select
              allowClear
              placeholder="Auto — oldest unpaid order first"
              value={orderId ?? undefined}
              onChange={(v) => setOrderId(v ?? null)}
              options={orderOptions.map((o) => ({
                value: o.order_id,
                label: `#${o.order_number} — Due: ${o.due_remaining.toFixed(2)}`,
              }))}
              className="w-full mt-1"
            />
          </div>
...
export default memo(CustomerQuickPaymentModal);
