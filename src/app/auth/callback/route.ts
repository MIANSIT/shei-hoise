...
export async function GET(request: Request) {
  ...
  if (code) {
    ...
    const supabase = createServerClient(
      ...
      {
        // Must match the browser client's explicit cookie name (see
        // src/lib/supabase/client.ts) — otherwise this derives its own name
        // from SUPABASE_INTERNAL_URL and never finds the session cookie the
        // browser actually set.
